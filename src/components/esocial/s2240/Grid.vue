<template>
  ...
  >
    ...
    <ButtonGroup slot="btn-actions" slot-scope="data">
      <Button
        :to="{ name: 'editarS2240', params: { id: data.key.id } }"
        size="small"
      >
        <Icon type="md-create" />
      </Button>
      <Button @click="deleteS2240(data.key)" size="small">
        <Icon type="md-close" />
      </Button>
    </ButtonGroup>
...
export default {
  ...
  watch: {
    value(newValue) {
      this.cadastross2240 = newValue || [];
    },
  },
  methods: {
    deleteS2240(data) {
      this.$emit("modalDeleteS2240", data);     
    },
  },
  created() {
    this.cadastross2240 = this.value || [];
  },
};
</script>
